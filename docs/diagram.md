...
| UC4 | Quản lý người dùng (tạo, đổi role, xóa) | Admin — đây là chức năng UI duy nhất của Admin |
...
| UC9 | Chat hỏi đáp RAG theo ngữ cảnh | **Chỉ Sinh viên** (Admin chỉ quản lý người dùng) |
| UC10 | Xem trích dẫn nguồn của câu trả lời | đi kèm UC9 («include») |
| UC11 | Quản lý phiên chat (tạo/ghim/đổi tên/xóa) của mình | |
| UC12 | Xem toàn bộ phiên chat mọi người dùng | Backend hỗ trợ Admin; UI hiện không hiển thị |
...
> **Admin chỉ quản lý người dùng** (UC4) + duyệt yêu cầu tài khoản (UC3) + cấu hình env (UC20).
> Giao diện Admin không có chat/quiz/phòng/tài liệu — backend vẫn có thể giám sát (UC12) nhưng UI ẩn.
...
| **User** | id: int, email: str (unique), password_hash: str, full_name: str, role_id: int → Role, created_at: datetime |
...
1. **users**: **id**, email UNIQUE INDEX, password_hash, full_name, role_id → roles.id, created_at
...
Participants: Student → React UI → ChatRouter → RateLimiter + require_role(USER, ADMIN) → ChatService → RagFacade(Embedder + ChromaDB) → GeminiClient → SQLite.
1. `POST /api/chat {question, session_id?, course_id?}`.
2. Guard: Lecturer → 403. RateLimiter giới hạn cố định 30 câu/phút (chỉ áp cho role USER) → quá → 429.
...
- **Backend FastAPI (Modular Monolith)**:
  ...
  - Business modules: `auth`, `account_requests`, `users` (kèm bảng `roles`), `courses`, `documents`, `chat`, `quizzes`, `rooms`.
  - Shared services: `rag` (Embedder, VectorStore, Retriever — Facade), `llm` (GeminiClient), `shared/rate_limit` (RateLimiter giới hạn cố định cho SV + IPRateLimiter), `shared/mailer` (Brevo API / Gmail SMTP — gửi email duyệt tài khoản cho người dùng + email báo Admin khi có yêu cầu mới), `shared/dependencies` (get_current_user, require_role).
...
- documents → rag (ingest); chat → rag + llm + rate_limit; rooms → quizzes + documents + courses + users; account_requests → users (create_account) + mailer; auth → users.
...
| POST | /api/chat | **USER, ADMIN** | Lecturer → 403; rate-limit cố định cho SV |
...
- Rate-limit: chat giới hạn cố định cho Sinh viên (USER only — ~30 câu/phút); form yêu cầu tài khoản 5/giờ/IP.
