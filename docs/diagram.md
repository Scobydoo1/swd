...
| Lớp | Thuộc tính |
|-----|-----------|
| **User** | id: int, email: str (unique), password_hash: str, full_name: str, role: Role, plan: Plan, created_at: datetime |
| **AccountRequest** | id: int, email: str, full_name: str, requested_role: Role (≠ADMIN), message: str, status: RequestStatus, created_at: datetime, decided_at: datetime? |
...
1. **users**: **id**, email UNIQUE INDEX, password_hash, full_name, role ENUM, plan ENUM, created_at
2. **account_requests**: **id**, email INDEX, full_name, requested_role ENUM, message, status ENUM, created_at, decided_at NULL
...
4. Lưu AccountRequest(status=PENDING) → 201.
5. Admin mở tab "Yêu cầu chờ duyệt" → `GET /api/account-requests?status=PENDING`.
...
- **Backend FastAPI (Modular Monolith)**:
  ...
  - Shared services: `rag` (Embedder, VectorStore, Retriever — Facade), `llm` (GeminiClient), `shared/rate_limit` (RateLimiter theo plan + IPRateLimiter), `shared/mailer` (Brevo API / Gmail SMTP), `shared/dependencies` (get_current_user, require_role).
